...
def _new_tree() -> SessionTree:
    header = SessionHeader(id="sid-export", workspace_id="ws", agent_id="default", session_key="key")
...
def test_markdown_renders_compaction_entry() -> None:
    ...
    _append(tree, CompactionEntry(
        id=eid, parent_id=None, timestamp=now_iso(),
...
def test_markdown_pairs_tool_calls_with_tool_responses() -> None:
    ...
    user_id = generate_entry_id(set())
    _append(tree, MessageEntry(
        id=user_id, parent_id=None, timestamp=now_iso(), role="user",
        content="run a tool",
    ))
    asst_id = generate_entry_id({user_id})
    _append(tree, MessageEntry(
        id=asst_id, parent_id=user_id, timestamp=now_iso(), role="assistant",
        content="Calling tool",
        tool_calls=[
            {"id": "tc-1", "type": "function", "function": {"name": "echo", "arguments": '{"text": "hi"}'}}
        ],
    ))
    tool_id = generate_entry_id({user_id, asst_id})
    _append(tree, MessageEntry(
        id=tool_id, parent_id=asst_id, timestamp=now_iso(), role="tool",
        content="echo: hi",
        tool_call_id="tc-1",
    ))
...
def test_markdown_truncates_image_blocks_without_full_base64() -> None:
    ...
    eid = generate_entry_id(set())
    _append(tree, MessageEntry(
        id=eid, parent_id=None, timestamp=now_iso(), role="user",
        content=[
...
def test_json_serialization_round_trips() -> None:
    ...
    eid = generate_entry_id(set())
    _append(tree, MessageEntry(
        id=eid, parent_id=None, timestamp=now_iso(), role="user", content="hi",
    ))
