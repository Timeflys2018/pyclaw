...
## Sprint 3 — Tier 4 层 precedence + 渠道差异

Sprint 3 把 tier resolution 升级为 4 层(高到低):

1. **Per-message override** — Web SPA tier pill (localStorage 持久) / Feishu `/tier <tier> --once`
2. **Per-sessionKey override** — `/tier <tier>` 持久化到 Redis(**仅 Feishu 用**,见下)
3. **Per-user `UserProfile.tier_default`** — Sprint 3 新增,通过 `/admin user set` 配置
4. **Channel deployment default** — `settings.channels.{web,feishu}.default_permission_tier`

首个非 None 值生效。Per-server `forced_tier`(Sprint 2)叠加在以上之上,只能**收紧**,不能放宽。

### Layer 2 (per-sessionKey) 是 Feishu-only by design

Web 渠道**故意不**用 Layer 2 — 因为 Web SPA 有 tier pill 控件,localStorage 持久化 + per-message 覆盖已经覆盖所有用例。在 `web/chat.py` 第 445 行 `session_tier=None` 是硬编码。

Feishu 没有 UI 控件,只有 `/tier <tier>` slash 命令。Layer 2 让飞书用户在某个 chat 里发一次 `/tier yolo` → 写入 `pyclaw:feishu:tier:{sessionKey}` Redis key (TTL 7 天)→ 后续这个 chat 所有消息都 yolo,**不传染**到其他 chat / 其他用户,`/new` 不重置(Sprint 1 invariant)。

如果将来 Web 增加 `/tier` slash 命令,需要同时把 `web/chat.py` 第 445 行的 `session_tier=None` 改成读 `get_session_tier`。
