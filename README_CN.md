...
| **MCP 客户端** | ✅ | stdio 服务器 · `{server}:{tool}` 命名空间 · 信任级别映射含 operator override（仅可降级）· `/mcp list / restart / logs` · 非阻塞启动 + `/health` advisory · `{env:VAR}` 密钥占位符。见 [MCP 指南](./docs/zh/mcp.md) |
...
**测试统计：** 2168 单元/集成测试 + 10 E2E · ~12K 行 Python · 110 个源文件
...
- **[MCP 服务器（中文）](./docs/zh/mcp.md)** · **[MCP servers (EN)](./docs/en/mcp.md)** — Anthropic Model Context Protocol 集成：stdio server 配置、`{env:VAR}` 密钥、per-server `forced_tier`（仅可降级）、`/mcp list/restart/logs`、非阻塞启动、安全要点
...
- ✅ **MCP 客户端** — Anthropic Model Context Protocol 集成：stdio 服务器、双名 adapter 模式、per-server `forced_tier`（仅可降级）、`/mcp list/restart/logs`、非阻塞启动、`/health` advisory `mcp` 字段、日志密钥 redact（Sprint 2）
